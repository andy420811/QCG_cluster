...
do
	...
	do	
		...
		##cd s"$j"_w1.0
		sed -i "s/crest input.xyz --qcg water.xyz --chrg 0 --uhf 0 --nsolv 60 --T 25 --gsolv --nclus 20 --gbsa water > crest.out/crest input.xyz --qcg water.xyz --chrg 0 --uhf 0 --nsolv "$j" --T 40 --gsolv --nclus 20 --gbsa water --wscal 1.0 > crest.out/" sub.sh
		echo $i $j

		##sed -i "s/--nsolv 60/--nsolv "$j" --wscal 1.0 /" sub.sh		

		if ! qsub -N s"$j"_run"$i"_w1.0 sub.sh 
		then
			printf ""$i"\n"$j"\n" > lastrun.txt	
			exit 1
		fi
